  ...
  return (
    <section className={`py-20 px-6 text-center ${isDarkMode ? "bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]" : "bg-white text-[#262B32]"}`}>
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: false }}>
        <motion.h2 variants={fadeInUp} className="text-2xl font-bold mb-2">{t("See ShopAlly in Action")}</motion.h2>
        <motion.p variants={fadeInUp} className="mb-8">{t("Affordable shopping made simple")}</motion.p>

        <motion.div variants={fadeInUp} className={`max-w-5xl mx-auto p-12 rounded-xl shadow-lg relative ${isDarkMode ? "bg-[var(--color-card-bg)]" : "bg-[#F7F9FB]"}`}>
